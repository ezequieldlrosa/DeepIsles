...
class IslesEnsemble:
    ...
    def inference(self):
        ...
        if self.input_flair_path is not None and not self.fast:
            ...
            command_nvauto = f'{sys.executable} process.py --input_path {self.tmp_out_dir} --model_path {self.weights_dir}'
            commands.append((command_nvauto, path_nvauto))

            # FACTORIZER Command
            print_run('SWAN')
            path_factorizer = os.path.join(self.ensemble_path, 'src', 'FACTORIZER/')
            command_factorizer = f'{sys.executable} process.py --input_path {self.tmp_out_dir} --model_path {self.weights_dir}'
